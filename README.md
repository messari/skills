...
Messari's hosted MCP server. Gives any MCP-compatible AI client (Claude Code, Cursor, Codex, OpenClaw, etc.) direct access to Messari's data stack:
...
An [OpenClaw](https://openclaw.ai) / [ClawHub](https://clawhub.ai) skill that wraps the MCP server. Provides configuration, routing guidance, and API reference documentation so AI agents can efficiently use Messari's services. This is a thin onboarding layer — the MCP server handles all data access and tool execution.
